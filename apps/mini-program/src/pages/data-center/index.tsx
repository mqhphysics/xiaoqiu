...
import type { CompetitionDataResponse, PlayerStats, StandingRow } from '../../features/product/product.types'
...
export default function DataCenterPage() {
  ...
  return (
    <PublicShell active="data" tournamentId={tournamentId}>
      ...
      {state.phase === 'failed' && (
        <DataState kind="error" title="赛事数据不可用" description={state.message} onRetry={() => void load()} />
      )}
      {state.phase === 'ready' && (
        <View>
            ...
            <View className="season-switcher">
              ...
              <View className="season-switcher__buttons">
                {state.data.seasons.map((season) => (
                  <Button
                    className={'season-button ' + (season.tournamentId === state.data.tournament.id ? 'season-button--active' : '')}
    ...
    <View>
                ...
                <View className="overview-group" key={group.id}>
                  ...
                  {group.standings.slice(0, 3).map((row) => (
                    <View className="overview-group__row" key={row.teamId}>
                      <Text>{row.rank}</Text><Text>{row.teamName}</Text><Text>{row.points} 分</Text>
                    </View>
                  ))}
                </View>
              ))}
              <Button className="data-more-button" onClick={onGoStandings}>查看完整积分榜</Button>
            </View>
          )}
        </View>
            ...
            {data.leaders.scorers.slice(0, 5).map((player, index) => (
              <View className="overview-leader" key={player.id}>
                ...
                <View className="overview-leader__copy">
                  <Text>{player.displayName}</Text><Text>{player.team?.name ?? '暂无球队'}</Text>
                </View>
                <Text className="overview-leader__value">{player.goals}</Text>
              </View>
            ))}
            <Button className="data-more-button" onClick={onGoLeaders}>查看球员榜</Button>
          </View>
        </View>
      </View>
      ...
      {data.bracket.length > 0 && (
        <View className="data-overview-bracket">
          <ProductSection kicker="KNOCKOUT" title="淘汰赛进程" note={data.bracket.length + ' 个轮次'} />
          <View className="overview-bracket-rounds">
            {data.bracket.map((round) => (
              <View className="overview-bracket-round" key={round.id}>
                <Text>{round.name}</Text><Text>{round.matches.length} 场</Text>
              </View>
            ))}
            <Button className="data-more-button data-more-button--inline" onClick={onGoBracket}>展开对阵</Button>
          </View>
        </View>
      )}
    </View>
...
function Kpi({ value, label, live = false }: { value: number; label: string; live?: boolean }) {
  return (
    <View className={'data-kpi ' + (live ? 'data-kpi--live' : '')}>
      <Text className="data-kpi__value">{value}</Text><Text className="data-kpi__label">{label}</Text>
    </View>
  )
}
...
function Standings({ groups }: { groups: CompetitionDataResponse['groups'] }) {
  if (groups.length === 0) {
    return <DataState kind="empty" title="本赛季没有小组积分榜" description="该赛季采用淘汰赛赛制。" />
  }
  return (
        ...
        {groups.map((group) => (
          <View className="standing-table surface" key={group.id}>
            ...
            <View className="standing-table__head">
              <Text>排名</Text><Text>球队</Text><Text>赛</Text><Text>净胜</Text><Text>积分</Text>
            </View>
            {group.standings.map((row) => <StandingTableRow key={row.teamId} row={row} />)}
          </View>
        ))}
      </View>
    </View>
...
function StandingTableRow({ row }: { row: StandingRow }) {
  return (
    <View className={'standing-table__row ' + (row.rank <= 2 ? 'standing-table__row--qualifying' : '')}>
      <Text>{row.rank}</Text>
      <View className="standing-table__team">
        <Text className="standing-table__color" style={{ backgroundColor: row.primaryColor ?? '#758079' }} />
        <Text>{row.shortName}</Text>
      </View>
      <Text>{row.played}</Text>
      <Text>{row.goalDifference > 0 ? '+' : ''}{row.goalDifference}</Text>
...
function Bracket({ data }: { data: CompetitionDataResponse }) {
  if (data.bracket.length === 0) {
    return <DataState kind="empty" title="暂无淘汰赛对阵" description="晋级席位确认后将在这里显示。" />
  }
  return (
    <View className="bracket-page">
      <ProductSection kicker="BRACKET" title="淘汰赛对阵" note="向右查看后续轮次" />
      <View className="bracket-scroll">
        {data.bracket.map((round) => (
          <View className="bracket-round" key={round.id}>
            <Text className="bracket-round__title">{round.name}</Text>
              ...
              {round.matches.map((match) => (
                ...
                >
                  <View className="bracket-match__meta">
                    <Text>{formatDate(match.scheduledStartAt)} {formatTime(match.scheduledStartAt)}</Text>
                    <MatchStatus status={match.status} />
                  </View>
                  <BracketTeam team={match.homeTeam} placeholder={match.homePlaceholder} score={match.homeScore} />
                  <BracketTeam team={match.awayTeam} placeholder={match.awayPlaceholder} score={match.awayScore} />
                </View>
              ))}
            </View>
          </View>
    ...
    <View className="leaders-page">
      <View className="leaders-heading">
        ...
        <View className="leader-toggle">
          <Button className={active === 'scorers' ? 'leader-toggle__active' : ''} onClick={() => onChange('scorers')}>射手榜</Button>
          <Button className={active === 'assists' ? 'leader-toggle__active' : ''} onClick={() => onChange('assists')}>助攻榜</Button>
        </View>
      </View>
      <View className="leader-table surface">
        <View className="leader-table__head">
          <Text>排名</Text><Text>球员</Text><Text>球队</Text><Text>出场</Text><Text>{active === 'scorers' ? '进球' : '助攻'}</Text>
        </View>
        {rows.map((player, index) => (
          ...
          >
            ...
            <View className="leader-table__player">
              <UserAvatar name={player.displayName} size="small" /><Text>{player.displayName}</Text>
            </View>
            <Text>{player.team?.shortName ?? '-'}</Text>
            <Text>{player.appearances}</Text>
            <Text className="leader-table__value">{active === 'scorers' ? player.goals : player.assists}</Text>
          </View>
        ))}
      </View>
    </View>
